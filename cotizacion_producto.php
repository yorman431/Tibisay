...
if ($_POST['enviar'] == "Cotizar"){
	if(!isset($calcular))
		$calcular= new Producto();
	$calcular->calcular_cotizacion();
}
	
if(isset($_GET['temp']) && $_GET['temp']!="" && isset($_GET['id']) && $_GET['id']!="" && isset($_GET['h']) && $_GET['h']!=""){
	$temporada=$_GET['temp']; $producto=$_GET['h']; $plan=$_GET['id'];
	
	if(!isset($cotizacion))
		$cotizacion= new Producto();
	$cotizacion->mostrar_producto_cotizacion($producto, $temporada, $plan);
	$cotizacion->mostrar_producto_publico($producto);
...
$smarty->assign("producto", $_GET['h']);
$smarty->assign("plan", $_GET['id']);


$smarty->force_compile=true;
$smarty->display('cotizacion2.tpl');
