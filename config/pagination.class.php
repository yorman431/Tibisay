    ...
    function links()
    {
      ...
      if (($this->pages) > 1)
      {
		...
		if ($this->page) {
		  ...
		  if ($this->page == 1)
		  	$plinks[] = ' <a class="inactivo">&laquo; Prev</a> ';
		  else
	    ...
	    if ($this->page) {
			if ($this->page < $this->pages)
				$slinks[] = ' <a href="?page='.($this->page + 1).$queryURL.'"> Next &raquo; </a> ';
          		...
          		$slinks[] = ' <a class="inactivo">Next &raquo;</a> ';
            if ($this->showFirstAndLast) {
		...
		$this->arreglo_completo[]=$plinks;$this->arreglo_completo[]=$links; $this->arreglo_completo[]=$slinks;
		//print_r($this->arreglo_completo);
		return $this->arreglo_completo;
