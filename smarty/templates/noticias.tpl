...
================================================== -->
  ...
  <nav class="navbar navbar-default navbar-fixed-top">
    ...
    <!-- Noticias -->
      ...
      {if $cont eq '0'}{assign var="cont" value=1}{else}{assign var="cont" value=0}{/if}
      
      	  {if $cont eq '0'}
          <div class="row featurette">
                <div class="col-md-7">
                  <h4>{$noticias[i].categoria_not} | {$noticias[i].fecha_not}</h4>
                  <h2 class="featurette-heading">{$noticias[i].titulo_not} <span class="text-muted">{$noticias[i].subtitulo_not}</span></h2>
                  <p class="lead">{$noticias[i].contenido_not}</p>
                  <p><a class="btn btn-success" title="{$noticias[i].titulo_not}" href="noticia_detalle.php?id={$noticias[i].id_not}" role="button">Leer m&aacute;s &raquo;</a></p>
                </div>
                <div class="col-md-5">
                    <div class="flexslider">
  						<ul class="slides">
                        	{section name=j loop=$noticias[i].imagenes}
                        	<li>
                    <a title="{$noticias[i].titulo_not}" href="noticia_detalle.php?id={$noticias[i].id_not}">
                        <img class="featurette-image img-responsive img-rounded" src="/imagenes/{$noticias[i].imagenes[j].directorio_image}" alt="{$noticias[i].imagenes[j].nombre_image}" width="400" height="400" />
                    </a>
                    		</li>
                            {/section}
                    	</ul>
