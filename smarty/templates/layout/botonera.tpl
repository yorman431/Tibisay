      ...
      <ul class="nav navbar-nav" style="padding-top:17px;">
        {section name=i loop=$enlaces_B}
            ...
            {if $enlaces_B[i].icono_cat neq ""}
              <li {if $enlaces_B[i].id_cat eq $activo}class="active"{/if}><a class="transicion" title="{$enlaces_B[i].etiqueta_cat}" {if $enlaces_B[i].etiqueta_cat eq "HOME"}href="/index.php"{else} href="#{$enlaces_B[i].etiqueta_cat} {/if}" id="efecto">{$enlaces_B[i].icono_cat}&nbsp;&nbsp;<span style="vertical-align:bottom; letter-spacing:0.2em;">{$enlaces_B[i].nombre_cat}</span></a></li>
            {else}
              <li {if $enlaces_B[i].id_cat eq $activo}class="active"{/if}><a class="transicion" title="{$enlaces_B[i].etiqueta_cat}" {if $enlaces_B[i].etiqueta_cat eq "HOME"}href="/index.php"{else} href="#{$enlaces_B[i].etiqueta_cat} "{/if} id="efecto">{$enlaces_B[i].nombre_cat}</a></li>
            {/if}
