...
function asigCantidad_Ninos(cantidad, id, idHab){
	...
	if(cantidad >= 1){
		...
		for (var i=1; i<=cantidad; i++){
			cadena += '<tr><th width="30%" valign="middle">Niño '+i+' </th><td><div class="input-group date"><input type="text" class="form-control" name="nino'+i+'" id="datetimepicke'+idHab+i+'" placeholder="Fecha de Nacimiento*" value=""><span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span></div><script type="text/javascript">$(function () {$(\'#datetimepicke'+idHab+i+'\').datetimepicker({viewMode: \'days\',format: \'DD-MM-YYYY\'});});</script></td></tr>';
		}
		$("#"+id).append(cadena);
	}else{
		$("#"+id).empty();
